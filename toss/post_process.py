...
def classify(atom_idx_list, res, length_tolerance=0.05):
    classes_dict = {}

    for i in atom_idx_list:
        group = "super" if i in res.super_atom_idx_list else "link"
        ele = get_ele_from_sites(i, res)
        shell = tuple(sorted(res.shell_ele_list[i]))
        avg_length = np.mean([res.matrix_of_length[i][j] for j in res.shell_idx_list[i]])

        found_group = None
        for key in classes_dict:
            # Unpack key
            ele_k, shell_k, group_k, length_k = key
            # Compare everything including tolerance
            if (ele, shell, group) == (ele_k, shell_k, group_k) and abs(avg_length - length_k) < length_tolerance:
                found_group = key
                break

        if found_group:
            classes_dict[found_group].append(i)
        else:
            # New group
            new_key = (ele, shell, group, avg_length)
            classes_dict[new_key] = [i]
    ...
    for pair_name,info in temp_pair_info.items():
        ...
        if pair_name in pred_dict:
            ...
            for label,length_list in info.items():
                ...
                """
                if nl <= 2000:
                    nl = max(sum([v[1] for k,v in useful_pair.items() if k[1] == label[1]]), 1)
                    NL = sum([v[1] for k,v in useful_pair.items()])
                """
                ...
                try:
                    mean = round(global_mean_dict[key],3)
                    sigma = round(global_sigma_dict[key],3)
                    sigma = 0.005 if sigma == 0 else sigma
                except Exception as e:
