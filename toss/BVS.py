...
def load_element_data(excel_path):
    openexcel = pd.read_excel(excel_path, sheet_name="Radii_X")
    ...
    # Load IP data and oxidation state ranges
    openexcel_ip = pd.read_excel(excel_path, sheet_name="IP")
    temp_dict = pd.read_excel(excel_path, sheet_name="min_max",
    ...
    for k, v in dic_s.items():
        ...
        symbol = str(k)
        ele = Element(k)
        list_IP = openexcel_ip[k].values.tolist()


        element_data[symbol] = {'symbol': symbol, 'R1': covalent_radius, 'X': X,
            'IP': list_IP, 'R2': second_covalent_radius, 'R3': third_covalent_radius}
...
def cal_BVS(i, mid, path):
    cutoff_radius = 5
    OS = BVAnalyzer(max_permutations=2_000_000, max_radius=cutoff_radius)
    try:
        ...
        for idx, site in enumerate(struct):
            neighbors = struct.get_neighbors(site, cutoff_radius)
            CN_list.append(len(neighbors)) 
            
            sen = 0
            for neighbor in neighbors:
                neighbor_element = neighbor.specie.symbol
                neighbor_en = element_data_dict[neighbor_element]['X']
                sen += neighbor_en
                neighbor_info.append([idx, neighbor.index, neighbor.nn_distance])
        ...
        for idx, site in enumerate(struct):
            ...
            element_data = element_data_dict[element]
            R1 = element_data['R1']
            EN = element_data['X']
            R2 = element_data['R2']
            R3 = element_data['R3']
            IP_list = element_data['IP']
            valence = valence_list[idx]
            CN = CN_list[idx]
            SEN = SEN_list[idx]
            atomic_number = Element(element).Z
            
            node_features = [atomic_number, EN, CN, SEN, R1, R2, R3] + IP_list[:8] + [valence]
            node_feats.append(node_features)

        if neighbor_info:
            node_feats = np.array(node_feats)
            edge_feats = np.array(neighbor_info)

            node_df = pd.DataFrame(node_feats, columns=["Element", "EN", "CN", "SEN", "R1", "R2", "R3",
                                          "IP1", "IP2", "IP3", "IP4", "IP5", "IP6", "IP7", "IP8", "OS"])
            edge_df = pd.DataFrame(edge_feats, columns=["Src", "Dst", "Distance"])
            G = {"n": node_df, "e": edge_df}
            print("Get the %s-th Graph of the name %s successfully."%(i, mid))
        else:
            G = None
...
def assemble_BVS(parameters):
    ...
    if parameters[1] != None:
        ...
        graphs_dict.update({parameters[0]: parameters[2]})
    else:
...
structure_path = os.path.join(path,"structures")
element_data_dict = load_element_data("../pre_set.xlsx")
...
target_group = os.listdir(structure_path)
...
if __name__ == "__main__":
    ...
    file_save = open(path + "/BVS_graphs_dict.pkl",'wb') 
    ...
    # if add the next line, will only process the TOSS and BVS double checked result.
