...
"""END HERE"""
"""
	def apply_redox_new(self, classes_dict, valence_list, res,global_nomalized_dict):
		loss_matrix_dict = {}
		OS_limit = []
		ele_limit = []

		TOS = sum([valence_list[i] for i in res.super_atom_idx_list])

		for k,v in classes_dict.items():
			OS_limit.append((k,[os for os in range(res.min_oxi_list[v[0]], res.max_oxi_list[v[0]]+1)]))
			ele_limit.append(len(v)) 

		solution = enu_permu_combi([len(limit[1])-1 for limit in OS_limit])
		real_solution = []
		for s in solution:
			mlp = []
			for i,j in enumerate(s):
				mlp.append(OS_limit[i][1][j])
			if np.dot(mlp,ele_limit) == TOS:
				real_solution.append(mlp)

		candi_valence_list = []
		for s in real_solution:
			temp_valence_list = copy.deepcopy(valence_list)
			for i, os in enumerate(s):
				index = classes_dict[OS_limit[i][0]]
				for j in index:
					temp_valence_list[j] = os
			candi_valence_list.append(temp_valence_list)

		for temp_valence_list in candi_valence_list:
			try:
				temp_pair_info = spider_pair_length_with_CN_unnorm(temp_valence_list, res)
				LOSS = cal_loss_func_with_CN(temp_pair_info,global_nomalized_dict)
			except:
				LOSS = 100000
			loss_matrix_dict[LOSS] = temp_valence_list

		return loss_matrix_dict


	def tune_by_redox_in_certain_range_new(self, atom_idx_list, LOSS, valence_list, bound, res,global_nomalized_dict):
		classes_dict = classify(atom_idx_list,res)
		temp_loss = 0
		ori_LOSS = copy.deepcopy(LOSS)
		ori_sum_of_valence = copy.deepcopy(valence_list)
		temp_loss_list = [ori_LOSS]
		while temp_loss not in temp_loss_list: 
			#temp_loss = copy.deepcopy(LOSS)
			loss_matrix_dict = self.apply_redox_new(classes_dict, valence_list, res,global_nomalized_dict)
			print(loss_matrix_dict)
			LOSS = min(loss_matrix_dict.keys())
			temp_loss_list.append(LOSS)
			temp_loss = copy.deepcopy(LOSS)
			valence_list = loss_matrix_dict[min(loss_matrix_dict.keys())]
		if abs(LOSS - ori_LOSS) / ori_LOSS > bound:
			return LOSS, valence_list
		else:
			return ori_LOSS, ori_sum_of_valence
