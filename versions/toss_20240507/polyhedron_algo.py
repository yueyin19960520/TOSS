...
def set_valence_of_link_atoms(link_atom_idx_list, sum_of_valence, arti_n, res):
    ...
    for l in link_atom_idx_list:
        if res.min_oxi_list[l] > sum_of_valence[l]:
            #check_diff.append(1)
        #else:
            check_diff.append(l) 

    if len(check_diff) >0:
        n = np.sum(np.subtract([res.min_oxi_list[l] for l in check_diff], 
                               [sum_of_valence[l] for l in check_diff]))
        n += arti_n 
        if n < 0:
            raise ValueError
        for i in check_diff :
            sum_of_valence[i] = res.min_oxi_list[i]
    else:
        n = arti_n  
...
def over_oxided(link_atom_idx_list, sum_of_valence, n, res):
    ...
    solutions = enu_permu_combi(limit)

    candi = []
    for s in solutions:
        if np.dot(s, num_limit) == N:
            candi.append(s)

    energy_per_solution = []

    for solution in candi:
        sum_X = 0
        effective_val = 0
...
def jump_to_resonance(res,sum_of_valence, super_atom_idx_list, n):
    ...
    resonance_atom = [i for i in super_atom_idx_list if get_ele_from_sites(i,res) in set_value_ele_dict]
    if len(resonance_atom) > 0:
        while n > 0:
            sort_idx_v = sorted([[i, sum_of_valence[i]] for i in super_atom_idx_list], key = lambda x:x[1])
            op_i = sort_idx_v[-1][0]
            sum_of_valence[op_i] -= 1
            n -= 1
            #print("722", n)
        return True
    else:
...
class POLYHEDRON_ALGO():
    ...
    def main_part(self, res, delta_X):
        ...
        for i in super_atom_idx_list:
            if sum_of_valence[i] > res.max_oxi_list[i]:
                record = copy.deepcopy(res.max_oxi_list[i])
                n -= (sum_of_valence[i] - record)
                sum_of_valence[i] = record
        ...
        self.ori_n,\
        self.ori_super_atom_idx_list,\
        self.ori_sum_of_valence = copy.deepcopy(n),copy.deepcopy(super_atom_idx_list),copy.deepcopy(sum_of_valence)
